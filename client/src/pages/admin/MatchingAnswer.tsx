...
import { MatchingAnswer, MatchingAnswerFormData } from "../../types/matching-answer";

export default function MatchingAnswerPage() {
  const [rows, setRows] = useState<MatchingAnswer[]>([]);
  ...
  const [editing, setEditing] = useState<MatchingAnswer | null>(null);
  const [formData, setFormData] = useState<MatchingAnswerFormData>({ 
    answer_id: "", 
    question_id: "", 
    left_item: "", 
    right_item: "" 
  ...
  const loadList = async () => {
    ...
    setError("");
    try {
      const res: any = await matchingAnswerService.list();
      if (res?.status === 'success' && Array.isArray(res.data)) {
        setRows(res.data);
      } else {
        setError("Không tải được danh sách đáp án nối.");
      }
    } catch (err: any) {
  ...
  const loadQuestionsForSelect = async () => {
    try {
      const res: any = await matchingQuestionService.list();
      if (res?.status === 'success' && Array.isArray(res.data)) {
        setQuestions(res.data);
      }
    } catch (err) {
      console.error("Failed to load questions for matching answers", err);
    }
  };

  useEffect(() => {
    loadList();
    loadQuestionsForSelect();
  }, []);

  const openAddModal = () => {
    setEditing(null);
    setFormData({ answer_id: "", question_id: "", left_item: "", right_item: "" });
    setFormError("");
    setIsModalOpen(true);
  };

  const openEditModal = (row: MatchingAnswer) => {
    setEditing(row);
    setFormData({ 
  ...
  const handleSave = async () => {
    ...
    if (!formData.left_item.trim() || !formData.right_item.trim()) {
      setFormError("Vui lòng nhập đầy đủ Vế Trái và Vế Phải.");
      return;
    }
    ...
    try {
      if (editing) {
        await matchingAnswerService.update(editing.answer_id, { 
          question_id: formData.question_id,
          left_item: formData.left_item, 
          right_item: formData.right_item 
        });
      } else {
        await matchingAnswerService.create({
  ...
  const handleDelete = async (row: MatchingAnswer) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa đáp án nối "${row.answer_id}"?`)) {
      try {
        await matchingAnswerService.delete(row.answer_id);
        loadList();
      } catch (err: any) {
        alert(err.response?.data?.message || err.message || "Lỗi khi xóa đáp án nối.");
      }
    }
  };
  ...
  const filtered = rows.filter(
    (a) =>
      ...
      (a.left_item || "").toLowerCase().includes(search.toLowerCase()) ||
      (a.right_item || "").toLowerCase().includes(search.toLowerCase())
  );
    ...
    <>
      ...
      <div className="topic-wrap" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            ...
            <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#0f172a", lineHeight: 1.2 }}>
              Quản lý Đáp Án Nối (Matching Answers)
            </h2>
            <p style={{ margin: "6px 0 0", fontSize: 13, color: "#94a3b8" }}>
              Danh sách các cặp tùy chọn (vế trái - vế phải) của từng câu hỏi nối
            </p>
          </div>
        ...
        >
          ...
          >
            <span style={{ fontSize: 13, fontWeight: 600, color: "#334155" }}>Danh sách cặp nối</span>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "#f8fafc" }}>
                  {["Answer ID", "Question ID", "Vế Trái (Left)", "Vế Phải (Right)", "Thao tác"].map((h, i) => (
                    <th
                      key={i}
                      style={{
                        padding: "10px 20px",
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={5} style={{ padding: "48px 20px", textAlign: "center", color: "#94a3b8" }}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontWeight: 500 }}>Không tìm thấy đáp án nối</span>
                        <span style={{ fontSize: 12 }}>Thử từ khóa khác hoặc thêm cặp nối mới</span>
                      </div>
                    </td>
                  </tr>
                ) : (
                  filtered.map((a, index) => (
                    ...
                    >
                      <td style={{ padding: "13px 20px", color: "#64748b", fontWeight: 600, fontSize: 13 }}>
                        {a.answer_id}
                      </td>
                      ...
                      <td style={{ padding: "13px 20px" }}>
                        <span style={{ color: "#0f172a", fontWeight: 500 }}>{a.left_item}</span>
                      </td>
                      <td style={{ padding: "13px 20px" }}>
                        <span style={{ color: "#334155" }}>{a.right_item}</span>
                      </td>
          ...
          <div className="modal-overlay">
            <div className="modal-content fade-up">
              <h3 style={{ margin: "0 0 16px", fontSize: 18, color: "#0f172a" }}>
                {editing ? "Chỉnh sửa Cặp Nối" : "Thêm Cặp Nối"}
              </h3>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <select
                  ...
                  style={{
                    width: "100%", padding: "10px 14px", borderRadius: 8,
                    border: "1px solid #cbd5e1", fontSize: 14, outline: "none", transition: "border 0.2s"
                  }}
                >
                  <option value="">-- Chọn Question ID (loại matching) --</option>
                  {questions.map((q) => (
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 13, fontWeight: 500, color: "#475569" }}>
                  Vế Trái (Left Item) <span style={{ color: "red" }}>*</span>
                </label>
                <input
                  type="text"
                  value={formData.left_item}
                  onChange={(e) => setFormData({ ...formData, left_item: e.target.value })}
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 13, fontWeight: 500, color: "#475569" }}>
                  Vế Phải (Right Item) <span style={{ color: "red" }}>*</span>
                </label>
                <input
                  type="text"
                  value={formData.right_item}
                  onChange={(e) => setFormData({ ...formData, right_item: e.target.value })}
