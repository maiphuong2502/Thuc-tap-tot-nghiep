...
export default function FillUserAnswer() {
    const [answers, setAnswers] = useState<FillUserAnswer[]>([]);
    ...
    const fetchAnswers = async () => {
        setLoading(true);
        try {
            const res = await axios.get("/api/fill-user-answers");
    ...
    const filtered = answers.filter(a =>
        a.faw_id.toLowerCase().includes(search.toLowerCase()) ||
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
                <div>
                    <h1 style={{ fontSize: 26, fontWeight: 800, color: "#1e293b", margin: 0, letterSpacing: "-0.6px" }}>
                        Kết quả Fill-in-the-blank
                    </h1>
                    <p style={{ color: "#64748b", margin: "4px 0 0", fontSize: 14 }}>
                        Bài làm của học viên cho câu hỏi điền từ
                    </p>
                </div>
            </div>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
                        <thead>
                            <tr style={{ background: "#f8fafc", borderBottom: "1px solid #f1f5f9" }}>
                                ...
                                <th style={{ padding: "14px 20px", fontSize: 12, fontWeight: 600, color: "#64748b", textTransform: "uppercase" }}>Câu trả lời</th>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr key={ans.faw_id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                                    ...
                                    <td style={{ padding: "14px 20px", color: "#334155", fontSize: 14, maxWidth: 300 }}>
