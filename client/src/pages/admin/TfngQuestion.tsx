...
export default function TfngQuestionPage() {
  const [rows, setRows] = useState<TfngQuestion[]>([]);
  ...
  const loadList = async () => {
    ...
    try {
      ...
      if (res?.status === "success" && Array.isArray(res.data)) {
        ...
        setError("Không tải được danh sách câu hỏi TFNG.");
      }
    } catch (err: any) {
  ...
  const loadQuestionsForSelect = async () => {
    try {
      const res: any = await questionService.list({ type: "TFNG", per_page: 1000 });
      ...
      console.error("Failed to load questions for TFNG", err);
    }
  };
  ...
  const openEditModal = (row: TfngQuestion) => {
    setEditing(row);
  ...
  const handleSave = async () => {
    ...
    if (!formData.content.trim()) {
      setFormError("Vui lòng nhập nội dung câu hỏi TFNG.");
      return;
    }
    setSaving(true);
    setFormError("");
    try {
      if (editing) {
        await tfngQuestionService.update(editing.question_id, { content: formData.content });
      } else {
        await tfngQuestionService.create({
  ...
  const handleDelete = async (row: TfngQuestion) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa câu hỏi TFNG "${row.question_id}"?`)) {
      try {
        await tfngQuestionService.delete(row.question_id);
        loadList();
      } catch (err: any) {
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            ...
            <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#0f172a", lineHeight: 1.2 }}>
              Quản lý Câu Hỏi TFNG
            </h2>
            <p style={{ margin: "6px 0 0", fontSize: 13, color: "#94a3b8" }}>
              Nội dung câu hỏi dạng True / False / Not Given
            </p>
          </div>
                  ...
                  <tr>
                    <td colSpan={3} style={{ padding: "48px 20px", textAlign: "center", color: "#94a3b8" }}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontWeight: 500 }}>Không tìm thấy câu hỏi TFNG</span>
          ...
          <div className="modal-overlay">
            <div className="modal-content fade-up">
              <h3 style={{ margin: "0 0 16px", fontSize: 18, color: "#0f172a" }}>
                {editing ? "Chỉnh sửa Câu Hỏi TFNG" : "Thêm Câu Hỏi TFNG"}
              </h3>
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 13, fontWeight: 500, color: "#475569" }}>
                  Nội dung câu hỏi (True / False / Not Given) <span style={{ color: "red" }}>*</span>
                </label>
                <textarea
