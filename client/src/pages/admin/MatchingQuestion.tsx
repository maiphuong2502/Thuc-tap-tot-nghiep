...
import { MatchingQuestion } from "../../types/matching-question";

export default function MatchingQuestionPage() {
  const [rows, setRows] = useState<MatchingQuestion[]>([]);
  ...
  const [editing, setEditing] = useState<MatchingQuestion | null>(null);
  ...
  const loadList = async () => {
    ...
    try {
      ...
      if (res?.status === 'success' && Array.isArray(res.data)) {
        ...
        setError("Không tải được danh sách câu hỏi nối.");
      }
    } catch (err: any) {
  ...
  const loadQuestionsForSelect = async () => {
    try {
      const res: any = await questionService.list({ type: "MATCHING", per_page: 1000 });
      ...
      console.error("Failed to load questions for matching", err);
    }
  };
  ...
  const openEditModal = (row: MatchingQuestion) => {
    setEditing(row);
    setFormData({ question_id: row.question_id, content: row.content || "" });
    setFormError("");
    setIsModalOpen(true);
  };

  const handleSave = async () => {
    if (!formData.question_id && !editing) {
      setFormError("Vui lòng chọn ID câu hỏi.");
      return;
    }
    if (!formData.content.trim()) {
      setFormError("Vui lòng nhập nội dung câu hỏi nối.");
      return;
    }
    setSaving(true);
    setFormError("");
    try {
      if (editing) {
        await matchingQuestionService.update(editing.question_id, { content: formData.content });
      } else {
  ...
  const handleDelete = async (row: MatchingQuestion) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa câu hỏi nối "${row.question_id}"?`)) {
      try {
        await matchingQuestionService.delete(row.question_id);
        loadList();
      } catch (err: any) {
    ...
    <>
      ...
      <div className="topic-wrap" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            ...
            <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#0f172a", lineHeight: 1.2 }}>
              Quản lý Câu Hỏi Nối (Matching)
            </h2>
            <p style={{ margin: "6px 0 0", fontSize: 13, color: "#94a3b8" }}>
              Nội dung câu hỏi yêu cầu kỹ năng ghép nối cặp thông tin thích hợp
            </p>
          </div>
        ...
        >
          ...
          >
            <span style={{ fontSize: 13, fontWeight: 600, color: "#334155" }}>Danh sách câu hỏi nối</span>
                  ...
                  <tr>
                    <td colSpan={3} style={{ padding: "48px 20px", textAlign: "center", color: "#94a3b8" }}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontWeight: 500 }}>Không tìm thấy câu hỏi nối</span>
                        <span style={{ fontSize: 12 }}>Thử từ khóa khác hoặc thêm câu hỏi mới</span>
                      </div>
                    </td>
                  </tr>
                ) : (
                  filtered.map((q, index) => (
                    ...
                    >
                      ...
                      <td style={{ padding: "13px 20px" }}>
                        <span style={{ color: "#0f172a", whiteSpace: "pre-wrap" }}>{q.content}</span>
                      </td>
          ...
          <div className="modal-overlay">
            <div className="modal-content fade-up">
              <h3 style={{ margin: "0 0 16px", fontSize: 18, color: "#0f172a" }}>
                {editing ? "Chỉnh sửa Câu Hỏi Nối" : "Thêm Câu Hỏi Nối"}
              </h3>
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 13, fontWeight: 500, color: "#475569" }}>
                  Mô tả / Đề bài (vd: "Match the words with their meanings") <span style={{ color: "red" }}>*</span>
                </label>
                <textarea
