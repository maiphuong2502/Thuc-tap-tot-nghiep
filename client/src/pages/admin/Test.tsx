...
export default function Test() {
  ...
  const loadTests = async (keyword = "") => {
    setLoading(true);
    setError("");
    try {
      const res: any = await testService.list(keyword);
      if (res?.success && Array.isArray(res.data)) {
        setTests(res.data);
      } else {
        setError("Không tải được danh sách bộ đề.");
      }
    } catch (err: any) {
      setError(err.response?.data?.message || "Có lỗi xảy ra khi tải danh sách bộ đề.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEditModal = (test: ITest) => {
    setEditingTest(test);
    setFormData({ test_name: test.test_name, description: test.description || "" });
    setFormError("");
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingTest) {
        await testService.update(editingTest.test_id, formData);
      } else {
  ...
  const handleDelete = async (test: ITest) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa bộ đề "${test.test_name}"?`)) {
      try {
        await testService.delete(test.test_id);
        loadTests(search);
      } catch (err: any) {
        alert(err.response?.data?.message || "Lỗi khi xóa bộ đề.");
      }
    }
  };
  ...
  return (
    <>
      <style>{`
        ...
        .test-wrap * { box-sizing: border-box; font-family: 'Be Vietnam Pro', sans-serif; }

        .test-row { transition: background 0.15s; }
        .test-row:hover { background: #f0f7ff !important; }
      ...
      <div className="test-wrap" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16 }}>
          <div>
            ...
            <p style={{ margin: "6px 0 0", fontSize: 13, color: "#94a3b8" }}>
              Danh mục bộ đề dùng để tổ chức thi
            </p>
          </div>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: 18, fontWeight: 700, color: "#1d4ed8" }}>{tests.length}</span>
              <span style={{ fontSize: 12, color: "#3b82f6", fontWeight: 500 }}>bộ đề</span>
            </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "#f8fafc" }}>
                  {["ID", "Tên bộ đề", "Mô tả", "Thao tác"].map((h, i) => (
                    <th key={i} style={{
                  ...
                  <tr>
                    <td colSpan={4} style={{ padding: "48px 20px", textAlign: "center", color: "#94a3b8" }}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontWeight: 500 }}>Chưa có bộ đề nào</span>
                  ...
                  tests.map((test, index) => (
                    ...
                    >
                      ...
                      <td style={{ padding: "13px 20px", color: "#64748b" }}>
                        {test.description || <span style={{ fontStyle: "italic", color: "#cbd5e1" }}>Không có</span>}
                      </td>
                      <td style={{ padding: "13px 20px", textAlign: "right", whiteSpace: "nowrap" }}>
                        <button onClick={() => openEditModal(test)} style={{ background: "none", border: "none", cursor: "pointer", color: "#3b82f6", marginRight: 10 }}>✏️ Sửa</button>
                        <button onClick={() => handleDelete(test)} style={{ background: "none", border: "none", cursor: "pointer", color: "#ef4444" }}>🗑️ Xóa</button>
                      </td>
                    </tr>
                  ))
                )}
          ...
          <div className="modal-overlay">
            <div className="modal-content fade-up">
              <h3 style={{ margin: "0 0 16px", fontSize: 18, color: "#0f172a" }}>
                {editingTest ? "Chỉnh sửa bộ đề" : "Thêm bộ đề mới"}
              </h3>
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 13, fontWeight: 500, color: "#475569" }}>
                  Tên bộ đề <span style={{ color: "red" }}>*</span>
                </label>
                <input
                  type="text"
