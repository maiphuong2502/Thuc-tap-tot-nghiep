...
export default function FillQuestionPage() {
  ...
  const loadFillQuestions = async () => {
    ...
    try {
      ...
      if (res?.success && Array.isArray(res.data)) {
        ...
        setError("Không tải được danh sách câu hỏi điền từ.");
      }
    } catch (err: any) {
  ...
  const loadQuestionsForDropdown = async () => {
    try {
      const res: any = await questionService.list({ type: 'FILL', per_page: 1000 });
  ...
  const openEditModal = (fq: FillQuestion) => {
    setEditingQuestion(fq);
    setFormData({ question_id: fq.question_id, content: fq.content || "" });
    setFormError("");
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (!formData.content.trim()) {
      setFormError("Vui lòng nhập nội dung câu hỏi điền từ.");
      return;
    }
    setSaving(true);
    setFormError("");
    try {
      if (editingQuestion) {
        await fillQuestionService.update(editingQuestion.question_id, formData);
      } else {
        await fillQuestionService.create(formData);
      }
  ...
  const handleDelete = async (fq: FillQuestion) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa câu hỏi điền từ "${fq.question_id}"?`)) {
      try {
        await fillQuestionService.delete(fq.question_id);
        loadFillQuestions();
      } catch (err: any) {
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            ...
            <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#0f172a", lineHeight: 1.2 }}>
              Quản lý Câu Hỏi Điền Từ
            </h2>
            <p style={{ margin: "6px 0 0", fontSize: 13, color: "#94a3b8" }}>
              Danh sách chi tiết nội dung các câu hỏi điền vào chỗ trống
            </p>
          </div>
                  ...
                  <tr>
                    <td colSpan={3} style={{ padding: "48px 20px", textAlign: "center", color: "#94a3b8" }}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontWeight: 500 }}>Không tìm thấy câu hỏi điền từ</span>
          ...
          <div className="modal-overlay">
            <div className="modal-content fade-up">
              <h3 style={{ margin: "0 0 16px", fontSize: 18, color: "#0f172a" }}>
                {editingQuestion ? "Chỉnh sửa Câu Hỏi Điền Từ" : "Thêm Câu Hỏi Điền Từ"}
              </h3>
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 13, fontWeight: 500, color: "#475569" }}>
                  Nội dung (Có chỗ trống, VD: "____") <span style={{ color: "red" }}>*</span>
                </label>
                <textarea
