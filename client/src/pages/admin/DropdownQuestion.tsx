...
export default function DropdownQuestionPage() {
  const [rows, setRows] = useState<DropdownQuestion[]>([]);
  ...
  const loadList = async () => {
    ...
    try {
      ...
      if (res?.success && Array.isArray(res.data)) {
        ...
        setError("Không tải được danh sách câu hỏi chọn từ.");
      }
    } catch (err: any) {
  ...
  const loadQuestionsForSelect = async () => {
    try {
      const res: any = await questionService.list({ type: "DROPDOWN", per_page: 1000 });
      ...
      console.error("Failed to load questions for dropdown", err);
    }
  };
  ...
  const openEditModal = (row: DropdownQuestion) => {
    setEditing(row);
  ...
  const handleSave = async () => {
    ...
    if (!formData.content.trim()) {
      setFormError("Vui lòng nhập nội dung câu hỏi chọn từ.");
      return;
    }
    setSaving(true);
    setFormError("");
    try {
      if (editing) {
        await dropdownQuestionService.update(editing.question_id, { content: formData.content });
      } else {
        await dropdownQuestionService.create({
  ...
  const handleDelete = async (row: DropdownQuestion) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa câu hỏi chọn từ "${row.question_id}"?`)) {
      try {
        await dropdownQuestionService.delete(row.question_id);
        loadList();
      } catch (err: any) {
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            ...
            <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#0f172a", lineHeight: 1.2 }}>
              Quản lý Câu Hỏi Chọn Từ
            </h2>
            <p style={{ margin: "6px 0 0", fontSize: 13, color: "#94a3b8" }}>
              Nội dung câu hỏi dạng chọn từ (dropdown) gắn với câu hỏi loại DROPDOWN
            </p>
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "#f8fafc" }}>
                  {["Question ID", "Skill ID", "Group ID", "Nội dung", "Thao tác"].map((h, i) => (
                    <th
                      key={i}
                      style={{
                        padding: "10px 20px",
                        ...
                        width: i === 0 || i === 1 || i === 2 ? 120 : i === 4 ? 150 : "auto",
                        whiteSpace: i === 0 || i === 1 || i === 2 ? "nowrap" : "normal",
                      }}
                    >
                      {h}
                    </th>
                  ...
                  <tr>
                    <td colSpan={5} style={{ padding: "48px 20px", textAlign: "center", color: "#94a3b8" }}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontWeight: 500 }}>Không tìm thấy câu hỏi</span>
          ...
          <div className="modal-overlay">
            <div className="modal-content fade-up">
              <h3 style={{ margin: "0 0 16px", fontSize: 18, color: "#0f172a" }}>
                {editing ? "Chỉnh sửa Câu Hỏi Chọn Từ" : "Thêm Câu Hỏi Chọn Từ"}
              </h3>
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 13, fontWeight: 500, color: "#475569" }}>
                  Nội dung (có chỗ chọn từ, ví dụ &quot;______&quot;) <span style={{ color: "red" }}>*</span>
                </label>
                <textarea
